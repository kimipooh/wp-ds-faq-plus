...
**/
    ...
    // 管理画面内でのアクセスのみURLをhttpsに書き換え
        ...
        $this->wp_ds_faq_plus_default_array['wp_ds_faq_plus_ver']      = '1480'; // 2011.08.29 (1.0.12): Version 
...
//            if($book[0]['mode'] == "1"){
            if($mode == "1"){ // 2011.07.19: 1.0.9 for sort
                if(is_array($quest)){
                    $results .= '<ol class="dsfaq_ol_quest">';
                    foreach ($quest as $s) { 
                        $results .= '<li id="dsfaq_quest_'.esc_attr($s['id']).'"><a href="#_" onclick="dsfaq_open_quest('.esc_attr($s['id']).');">'.strip_tags($s['quest']).'</a></li>';
